/**
 * navbar.constants.ts
 * Module-level static data for the Navbar — per APP_SHELL.md pattern.
 * No JSX, no React — safe to import from any context.
 */

import type { Notification } from '@/components/ui/notifications-modal'

export interface NavItem {
    label: string
    href: string
}

export const NAV_LINKS: NavItem[] = [
    { label: 'Services', href: '#services' },
    { label: 'Tours', href: '#featured-tours' },
    { label: 'Pricing', href: '#pricing' },
    { label: 'About', href: '#about-section' },
]

// Placeholder — replace with real notifications hook when backend API is ready
export const MOCK_NOTIFICATIONS: Notification[] = [
    {
        id: '1',
        title: 'Flight Confirmed',
        body: 'Your booking to Dubai (DBX) has been confirmed.',
        time: '2 mins ago',
        read: false,
    },
    {
        id: '2',
        title: 'New Quote Available',
        body: 'We have prepared the quote for your group trip to Bali.',
        time: '1 hour ago',
        read: false,
    },
    {
        id: '3',
        title: 'Welcome to Thrive!',
        body: 'Your account has been successfully created.',
        time: '1 day ago',
        read: true,
    },
]
