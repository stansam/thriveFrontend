...
import { useLocationSearch } from "@/lib/hooks/features/use-location-search";
import { CABIN_OPTIONS } from "../../../_constants/landing/search-flights.constants";
import { SearchFlightsFormProps } from "../../../_props/landing/search-flights.props";

export function SearchFlightsForm({
  className,
  onSearch,
}: SearchFlightsFormProps) {
  ...
  return (
    <Card
      className={cn(
        "w-[400px] p-6 bg-black/80 backdrop-blur-md border border-white/20 text-white shadow-2xl",
        className
      )}
    >
      <Form {...form}>
        <form onSubmit={handleSubmit(onSearch)} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <LocationInput
              ...
              id="from"
              form={form}
              inputValue={origin.inputValue}
              onInputChange={(v) => {
                origin.setInputValue(v);
                origin.setShowDropdown(true);
              }}
              onFocus={() => origin.setShowDropdown(true)}
              onBlur={() => setTimeout(() => origin.setShowDropdown(false), 200)}
              showDropdown={origin.showDropdown}
              results={origin.results}
              isSearching={origin.isSearching}
              debouncedValue={origin.debouncedValue}
            ...
            <LocationInput
              ...
              id="to"
              form={form}
              inputValue={destination.inputValue}
              onInputChange={(v) => {
                destination.setInputValue(v);
                destination.setShowDropdown(true);
              }}
              onFocus={() => destination.setShowDropdown(true)}
              onBlur={() =>
                setTimeout(() => destination.setShowDropdown(false), 200)
              }
              showDropdown={destination.showDropdown}
              results={destination.results}
              isSearching={destination.isSearching}
              debouncedValue={destination.debouncedValue}
